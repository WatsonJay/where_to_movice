...
def get_one_parse(url):
   with open('temp/temp.txt', 'a+') as f: # 保存在相应的文件里
       print(url) # 看爬取到第几页来了
...
def proxyip():
    csv_file_read1 = open('temp/validate.csv', 'r')
    csv_file_read = open('temp/validate.csv', 'r')
...
def main():
    csv_file = open('temp/validate.csv', 'w', newline='')
